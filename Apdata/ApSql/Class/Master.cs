...
namespace ApSql.Class
{
    internal class Master
    {
        ...
        public Master() { }
        public Master(ApSqlForm form, Server server, Query query, Run run, Xml xml)
        {
            try
            {
                Form = form;
                ...
                Parameter = Xml.LoadParameter();
                ClusterList = Xml.ClusterList;
            }
            catch(Exception msg)
            {
        ...
        internal void Start()
        {
            Form.clusterComboBox.DataSource = ClusterList.Select(x => x.Key).OrderBy(x => x).ToList();
        ...
        internal void LoadMainCluster()
        {
            ...
            cluster.Key = "All";
            cluster.DatabaseList = databaseInstanceList;

            ClusterList = ClusterList.Where(x => x.Key != "All").ToList();

            ClusterList.Add(cluster);

            Xml.SaveCluster(ClusterList);
        }
